...
```text
设计意图 → 暂停 → Idea 选择 → 暂停 → 随机种子 Brief → 暂停
→ 当前环境原生生图 → 暂停 → 界面实现 → 暂停
→ 独立截图评审 → 暂停 → 减法审计 → 最终验收
```

Skill 默认使用 Human-in-the-loop：每个回合最多完成一个阶段，收到用户明确批准后才继续。它会沿用现有项目的前端技术栈；如果是没有指定技术栈的独立轻量原型，则使用语义化 HTML、CSS 和 JavaScript。生图调用当前 Agent 环境提供的原生能力，暂不包含视频生成。
